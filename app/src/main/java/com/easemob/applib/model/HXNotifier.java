...
public class HXNotifier {
    ...
    public void sendMyNotification(EMMessage message ) {
        String username = message.getFrom();
        try {
            String notifyText = username + " ";
            switch (message.getType()) {
            ...
            // create and send notificaiton
            Intent msgIntent =appContext.getPackageManager().getLaunchIntentForPackage(packageName);
            msgIntent.setClass(appContext, MainActivity2.class);

            PendingIntent pendingIntent = PendingIntent.getActivity(appContext, 0, msgIntent,0);
